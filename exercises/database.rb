...
if response == 'c'
  ...
  puts "#{person[0][1]}'s number is #{person[0][2]}."
end
  # # look everyone up
if response == 'r'
...
if response == 'u'
  ...
  all.each do |entry|
    puts "#{entry[1]}'s number is #{entry[2]}. To select #{entry[1]}, enter the id of #{entry[0]} "
  end
  ...
  puts "#{person[0][1]}'s number is now #{person[0][2]}."
end
  # delete an entry
if response == 'd'
 # get a list of everyone so we can select the correct person by id
